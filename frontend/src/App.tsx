  ...
  return (
    <div className="app-shell">
      ...
      <main className="app-content">
        ...
        {page === "accounts" && <AccountsPage accounts={accountList} events={orderedEvents} onBack={() => selectPage("dashboard")} onCreate={(payload) => createAccount.mutateAsync(payload)} onUpdate={(id, payload) => updateAccount.mutateAsync({ id, payload })} onDelete={(id) => deleteAccount.mutate(id)} />}
        {page === "events" && <EventsPage accounts={accountList} events={orderedEvents} onBack={() => selectPage("dashboard")} onCreate={(payload) => createEvent.mutateAsync(payload)} onUpdate={(id, payload) => updateEvent.mutateAsync({ id, payload })} onDelete={(id) => deleteEvent.mutate(id)} />}
      </main>
    </div>
  );
}
...
function AccountForm({ initial, submitLabel = "계좌 추가", onSubmit, onCancel, isSaving, error }: { initial?: Account; submitLabel?: string; onSubmit: (payload: AccountPayload) => Promise<boolean>; onCancel?: () => void; isSaving: boolean; error: string }) {
...
function EventForm({ accounts, initial, submitLabel = "이벤트 추가", onSubmit, onCancel, isSaving, error }: { accounts: Account[]; initial?: Event; submitLabel?: string; onSubmit: (payload: EventPayload) => Promise<boolean>; onCancel?: () => void; isSaving: boolean; error: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); const saved = await onSubmit({ name: String(data.get("name")), event_date: String(data.get("date")), amount: String(data.get("amount")), type, status: initial?.status ?? "PLANNED", source_account_id: config.source ? String(data.get("source")) || null : null, destination_account_id: config.destination ? String(data.get("destination")) || null : null, recurrence_months: repeats ? Number(data.get("recurrenceMonths")) : null, recurrence_until: repeats ? String(data.get("recurrenceUntil")) || null : null, note: initial?.note ?? null }); if (saved) { form.reset(); setType("INCOME"); setRepeats(false); } }
  return <form onSubmit={submit}>
